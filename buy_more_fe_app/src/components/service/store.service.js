...
const getOrderToSetRoute = (storeID) => {
  // console.log(storeID)
  return axios.get('http://localhost:3001/store/setroutes', {
    params: storeID,
  });
};
...
const getOrdersOnTrain = (storeID) => {
  console.log(storeID);
  return axios.get('http://localhost:3001/store/ordersontrain', {
    params: storeID,
  });
};
...
const getAllRoutes = (storeID) => {
  // console.log(storeID)
  return axios.get('http://localhost:3001/store/scheduletruck', {
    params: storeID,
  });
};
...
const getDriversOnTrip = (storeID) => {
  return axios.get('http://localhost:3001/store/driverondelivery', {
    params: storeID,
  });
};

const changeState = (scheduleId, state, driverID, assistantID) => {
  return axios.post('http://localhost:3001/store/driverondelivery', {
    scheduleId,
    state,
    driverID,
    assistantID,
